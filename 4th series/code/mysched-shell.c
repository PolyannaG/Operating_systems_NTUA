...
{	
	...
	while (p!=running_process && (p->proc_pointer)->id!=id)  //case of shell:p=running process, other cases: loop until the asked id is found
		p=p->next;
	if (p==running_process && id!=0) {  //case of shell: p==running process, id==0: don't enter, other cases: scanned all the list, not found: return
	...
	for (;;) {
		...
		if (WIFSTOPPED(status)) {
			if (p==(running_process->proc_pointer)->pid){ 
				printf("Currently running process with id:%d and pid:%d has been stopped\n",(running_process->proc_pointer)->id, (running_process->proc_pointer)->pid);	
				run_next=1;
				...
				if (pr==NULL) 
					...
					printf("Process (not running) with id:%d and pid:%d has received stop signal.\n", (pr->proc_pointer)->id, (pr->proc_pointer)->pid);
				}
			}		
		}  
		if (WIFEXITED(status) || WIFSIGNALED(status)) {
			if (p==(running_process->proc_pointer)->pid) {
				printf("Currently running process with id:%d and pid:%d has exited or been killed.\n", (running_process->proc_pointer)->id, (running_process->proc_pointer)->pid);
				run_next=1;
				...
				proc_node *pr=get_process_with_pid(p);
				printf("Process not running with id:%d and pid:%d has exited or been killed.\n",(pr->proc_pointer)->id, (pr->proc_pointer)->pid);
				if (pr==pr->next) { //process that exited or was killed was the last process
					printf("Scheduler: No more processes to run.\n");
					exit(0);
				}
			}
...
int main(int argc, char *argv[])
{
	...
	running_process=running_process->next; //running_process was the last processes we addeed, make running process the first one (FIFO)
	...
	//if (nproc == 0) {                                                 //We don't need this part anymore!!! There is always a process when we call the program, the shell.
//		fprintf(stderr, "Scheduler: No tasks. Exiting...\n");
//		exit(1);
//	}
//	if (nproc>0) {
		kill((running_process->proc_pointer)->pid,SIGCONT);   //always wake a process up, we always have at least the shell
		alarm(SCHED_TQ_SEC);
//	}
